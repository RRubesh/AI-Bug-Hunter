...
class OllamaClient:
    ...
    async def verify_or_pull_model(self, model_name: str) -> bool:
        provider = settings.AI_PROVIDER
        if provider != "ollama":
            return True
    ...
    def _get_offline_fallback(self, category: str, message: str, code_snippet: str) -> Dict[str, str]:
        # Generates a highly comprehensive fallback explanation offline
        return {
            "explanation": (
    ...
    async def explain_vulnerability(self, category: str, message: str, code_snippet: str) -> Dict[str, str]:
        ...
        try:
            ...
            # Look for variations of remediation header (e.g. ### Secure Remediation, ### Remediation, etc.)
    ...
    async def chat_about_scan(self, chat_history: list, user_message: str, code_context: str) -> str:
        ...
        try:
            if settings.AI_PROVIDER == "ollama":
                models = await self.list_models()
                if not models:
                    return "The Ollama service is currently offline or unreachable. Please verify Ollama is running locally."
